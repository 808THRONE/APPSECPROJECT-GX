...
### Java 21 LTS Upgrade
The entire SecureGate backend ecosystem has been upgraded to **Java 21**, providing superior performance and access to modern JDK features.
- `api-gateway`: Fully ported and verified.
- `iam-service`: Fully ported and verified.
- `stego-module`: Built using Java 21.

### Steganography Module & Modern Encryption
We have implemented a robust steganography module for covert communication:
- **Algorithm**: **Syndrome Trellis Codes (STC)** based on the Filler-Fridrich methodology for near-optimal embedding efficiency and minimal distortion.
- **Encryption**: **XChaCha20-Poly1305** authenticated encryption (192-bit nonce, collision-resistant).
- **Quality Metrics**: Adaptive cost functions ensuring high imperceptibility (PSNR ≥ 45dB).
- **Integration**: Fully integrated into the Docker infrastructure with Traefik routing at `stego.${DOMAIN}`.
